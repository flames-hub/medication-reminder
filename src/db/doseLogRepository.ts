...
export async function getDoseLogsForDate(date: string): Promise<DoseLog[]> {
  ...
  return rows.map((r) => ({ ...r, skipped: r.skipped === 1, memo: r.memo ?? undefined }));
}
...
export async function upsertDoseLog(log: DoseLog): Promise<void> {
  const db = await getDatabase();
  await db.runAsync(
    'INSERT OR REPLACE INTO dose_logs (id, medicationId, scheduledAt, takenAt, skipped, memo) VALUES (?, ?, ?, ?, ?, ?)',
    [log.id, log.medicationId, log.scheduledAt, log.takenAt ?? null, log.skipped ? 1 : 0, log.memo ?? null]
  );
}
